...
public class DoubleMathUtils {
	 ...
	 * @param value
	 * @param scale
	 * @return sine 2014-4-4 上午9:18:58
	 */
	public static double round(double value, int scale) {
	...
	/**
	 * 把一个double类型转换成字符串,若不足两位小数,则用0不足,若超过两位小数,则原样返回其字符串样式
	 * @param money
	 * @return
	 * @throws Exception
	 */
	public static String formatDouble(Double money) throws Exception{
		if(money==null){
			throw new Exception("money不能为null");
		}
		NumberFormat bf=NumberFormat.getInstance();
		String a=bf.format(money);
		a=a.replaceAll(",", "");
		String b[]=a.split("\\.");
		if(b.length==1){
			return a+".00";
		...
		return b[0]+"."+c.charAt(0)+c.charAt(1);
	}
